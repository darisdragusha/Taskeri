...
class UserProjectRepository:
    ...
    def get_users_for_project(self, project_id: int) -> list[int]:
        """Get all user IDs assigned to a specific project."""
        return [
            up.user_id
            for up in self.db_session.query(UserProject).filter_by(project_id=project_id).all()
        ]
